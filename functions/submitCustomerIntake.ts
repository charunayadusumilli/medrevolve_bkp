...
<table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;">
  ...
  <tr><td style="background:#1a2a1a;border-radius:0 0 14px 14px;padding:14px 28px;text-align:center;">
    ...
    await Promise.all([
      sendEmail(base44, { to: data.email, subject: `✅ Welcome to MedRevolve, ${firstName}! Here's what happens next`, html: patientHtml }),
      sendEmail(base44, { to: adminEmail, subject: `🆕 New Customer Intake — ${data.full_name} [${data.primary_interest}]`, html: adminHtml }),
      sendEmail(base44, { to: adminEmail2, subject: `🆕 New Customer Intake — ${data.full_name} [${data.primary_interest}]`, html: adminHtml }),
    ...
    if (data.phone) {
      await sendSMS(data.phone, `MedRevolve: Hi ${firstName}! We got your intake for ${data.primary_interest}. We'll reach out in 24hrs. medrevolve.com/consultations`);
    }

    // SMS to admin with lead details
    const adminPhone = '5302006352';
    const phoneDisplay = data.phone || 'N/A';
    await sendSMS(adminPhone, `NEW LEAD - ${data.full_name} | ${data.primary_interest} | ${data.email} | ${phoneDisplay} | ${data.consultation_preference || 'flexible'} | via ${data.heard_about_us || 'N/A'}`);
