...
import { X, Send, Sparkles, ChevronDown, RotateCcw, PhoneCall, Zap, HeadphonesIcon, Bot, ZapOff, MessageSquare, BarChart3, Share2, Download, Star, Lightbulb, Target, TrendingUp, CheckCircle2, Clock, AlertCircle } from 'lucide-react';
        ...
        {isOpen && (
          ...
          >
            <div className="flex flex-col h-full rounded-2xl overflow-hidden shadow-2xl" style={{ background: '#fff' }}>
              ...
              {/* ── Header ── */}
              <div className="px-4 py-3 flex items-center justify-between flex-shrink-0" style={{ background: 'linear-gradient(135deg, #0A0A0A 0%, #1a1a1a 100%)' }}>
              <div className="flex items-center gap-2.5">
              {/* Avatar with AI indicator */}
              <div className="relative">
              <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#4A6741] to-[#6B8F5E] flex items-center justify-center text-white text-sm font-black shadow-lg">
              ...
              <p className="text-[10px] text-white/50 leading-tight flex items-center gap-1">
                ...
                <div className="flex items-center gap-1">
                  {/* AI Power Indicator */}
                  <div className="hidden sm:flex items-center gap-1.5 px-2 py-1 bg-white/5 rounded-lg border border-white/10">
                    <div className="flex gap-0.5">
                      {[0, 1, 2].map(i => (
                        <div key={i} className={`w-1 h-3 rounded-full ${loading ? 'bg-[#4A6741] animate-pulse' : 'bg-white/30'}`} style={{ animationDelay: `${i * 0.15}s` }} />
                      ))}
                    </div>
                  </div>
                  {voiceSupported && (
              ...
              {!minimized && (
                <>
                  ...
                  {/* ── AI Quick Start Cards — enhanced with icons ── */}
                  {showQuickStarts && ctx.suggestedPrompts?.length > 0 && messages.length <= 1 && (
                    <div className="flex-shrink-0 px-4 py-3 border-t border-gray-100" style={{ background: '#F9F9F7' }}>
                      <div className="flex items-center justify-between mb-2.5">
                        <div className="flex items-center gap-2">
                          <Lightbulb className="w-3.5 h-3.5 text-[#4A6741]" />
                          <p className="text-[10px] text-gray-500 font-semibold uppercase tracking-wide">Quick Start</p>
                        </div>
                        <button onClick={() => setShowQuickStarts(false)} className="text-gray-400 hover:text-gray-600">
                          <X className="w-3 h-3" />
                        </button>
                      </div>
                      <div className="flex flex-col gap-1.5">
                        {ctx.suggestedPrompts.slice(0, 3).map((prompt, i) => (
                          <button
                            key={i}
                            onClick={() => { setActiveSuggestion(i); sendMessage(prompt); }}
                            disabled={loading}
                            className={`text-[12px] px-3 py-2.5 rounded-xl font-medium border transition-all text-left disabled:opacity-50 flex items-center justify-between group ${
                              activeSuggestion === i 
                                ? 'bg-[#4A6741] text-white border-[#4A6741]' 
                                : 'bg-white text-gray-700 border-gray-200 hover:bg-[#4A6741] hover:text-white hover:border-[#4A6741]'
                            }`}
                          >
                            <span>{prompt}</span>
                            <ChevronDown className="w-3 h-3 opacity-0 group-hover:opacity-100 transition-opacity -rotate-90" />
                          </button>
                        ))}
                      </div>
                    </div>
                  ...
                  {/* ── Smart Input with AI Features ── */}
                  <div className="flex-shrink-0 px-3 py-3 bg-white border-t border-gray-100">
                    {/* AI Capability Badges */}
                    {messages.length <= 2 && (
                      <div className="flex gap-1.5 mb-3 flex-wrap">
                        <span className="px-2 py-1 bg-gradient-to-r from-[#4A6741]/10 to-[#6B8F5E]/10 rounded-lg border border-[#4A6741]/20">
                          <span className="text-[9px] font-semibold text-[#4A6741] flex items-center gap-1">
                            <Bot className="w-3 h-3" /> AI-Powered
                          </span>
                        </span>
                        <span className="px-2 py-1 bg-blue-50 rounded-lg border border-blue-200">
                          <span className="text-[9px] font-semibold text-blue-600 flex items-center gap-1">
                            <Zap className="w-3 h-3" /> Instant Responses
                          </span>
                        </span>
                        <span className="px-2 py-1 bg-purple-50 rounded-lg border border-purple-200">
                          <span className="text-[9px] font-semibold text-purple-600 flex items-center gap-1">
                            <Target className="w-3 h-3" /> Context-Aware
                          </span>
                        </span>
                      </div>
                    )}
                    ...
                    <div className="flex gap-2 items-center">
                      <Input
                        ...
                        placeholder={ctx.placeholder || "Ask about AI solutions..."}
                      ...
                      <button
                        ...
                        className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-all shadow-md ${
                          input.trim() 
                            ? 'bg-gradient-to-br from-[#4A6741] to-[#6B8F5E] text-white hover:shadow-lg hover:scale-105' 
                    ...
                    <div className="flex items-center justify-between mt-2.5 px-1">
                      <p className="text-[9px] text-gray-400 flex items-center gap-1">
                        ...
                        AI-powered responses · Business solutions
                      </p>
                      <button 
                        onClick={() => setShowQuickStarts(!showQuickStarts)}
                        className="text-[9px] text-[#4A6741] hover:text-[#6B8F5E] font-medium flex items-center gap-1"
                      >
                        <Lightbulb className="w-3 h-3" />
                        {showQuickStarts ? 'Hide' : 'Show'} suggestions
                      </button>
                    </div>
                  </div>
                </>
