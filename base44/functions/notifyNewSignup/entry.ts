...
Deno.serve(async (req) => {
  try {
    ...
    // Handle PatientIntake entity create events
    if (event.type !== 'create' || event.entity_name !== 'PatientIntake') {
      return Response.json({ skipped: true, reason: 'Not a PatientIntake create event' });
    }

    if (!data?.email) {
      return Response.json({ skipped: true, reason: 'No email in data' });
    }
      ...
      html: `
        <div style="font-family:sans-serif;max-width:600px;margin:0 auto;">
          ...
          <div style="background:#FDFBF7;padding:32px;border-radius:0 0 12px 12px;border:1px solid #E8E0D5;">
            ...
            <p style="color:#999;font-size:12px;text-align:center;">Questions? Email us at <a href="mailto:rned@medrevolve.com" style="color:#4A6741;">rned@medrevolve.com</a></p>
          </div>
        </div>
      `,
    });
    ...
    return Response.json({ success: true, message: 'Welcome email sent via Gmail' });
  } catch (error) {
    console.error('notifyNewSignup error:', error.message);
